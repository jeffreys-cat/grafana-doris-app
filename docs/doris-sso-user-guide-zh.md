...
| Grafana Datasource 管理员 | 创建 Datasource，配置 Doris 连接、OIDC issuer/audience，以及 OIDC group 到 Doris role 的映射；执行该 Datasource 生成的 Doris SQL。 |
...
| Default Doris role | 用户没有命中任何 group 映射时的最小默认权限，通常是 `doris_reader`。 |
...
- 用户命中多个 group 时，会获得所有命中 Doris role 的并集。
- 没有任何命中时，只获得 Default Doris role。
- 上游 group 不会原样传给 Doris；只有 Datasource 显式映射过的 Doris role 会进入 token。
...
在每次查询中，Datasource 会验证用户的 ID Token、映射 group、签发短期 Doris token，并以 JIT Doris 用户身份连接。Doris 用户名默认为上游 token 的 `sub`；用户不需要创建 Doris 密码账号。

如果用户看不到数据源、数据库或表，依次检查：

1. Grafana 是否授予该用户使用 Datasource 的权限；
2. ID Token 是否包含预期的 `groups` claim；
3. Datasource 的 group 映射是否精确匹配；
