...
3. 新建 Datasource 时，仅配置 Doris 连接、标准 **OIDC Discovery** issuer/audience、默认 Doris 角色和 group 映射。
...
- Datasource 可将已验证 ID Token 的标准 `groups` claim 映射到多个 Doris role：只有 Datasource 映射表中精确匹配的完整 group 路径会进入签发给 Doris 的 `doris_groups` claim，未命中时才授予默认只读角色。
- IDP 需要将用户组作为 `groups` claim 写入 ID Token，并保留完整路径，例如 `/team/readers`。Datasource 不会把未经配置的上游 group 透传给 Doris。
